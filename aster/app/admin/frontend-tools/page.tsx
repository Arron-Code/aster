...
const ADMIN_LAYOUT_OPTIONS: { id: AdminLayoutMode; label: string; tag: string; description: string }[] = [
  {
    id: "horizontal",
    label: "Horizontale Kopfzeile",
    tag: "Neu · Standard",
    description: "Menügruppen als horizontale Leiste im Seitenkopf – Unterpunkte öffnen sich per Hover als Dropdown, Benutzername und Abmeldung stecken im Benutzer-Symbol.",
  },
  {
    id: "sidebar",
    label: "Seitenleiste",
    tag: "Bisherige Darstellung",
    description: "Klassische Darstellung mit vertikaler Navigationsleiste auf der linken Seite.",
  },
];
...
export default function FrontEndToolsPage() {
  ...
  const [adminLayoutMode, setAdminLayoutMode] = useState<AdminLayoutMode>(DEFAULT_ADMIN_LAYOUT_MODE);
  ...
  useEffect(() => {
    ...
    setAdminLayoutMode(readAdminLayoutMode());
    const handleLayoutChange = (event: Event) => {
      const mode = (event as CustomEvent).detail;
      if (isAdminLayoutMode(mode)) setAdminLayoutMode(mode);
    };
  ...
  return (
    <>
      ...
      <div className="frontend-tool-tabs" role="tablist" aria-label="Untermenü Frontend-Tools">
        ...
        <button
          type="button"
          className={`frontend-tool-tab${activeTab === "admin" ? " active" : ""}`}
      ...
      {activeTab === "design" ? (
        ...
        <section className="card frontend-form-card">
          <h2>Admin-Bereich</h2>
          <p className="muted">
            Wähle die Darstellung des Admin-Bereichs. Die Auswahl gilt sofort und wird auf diesem Gerät gespeichert.
          </p>

          <div className="frontpage-version-list">
            {ADMIN_LAYOUT_OPTIONS.map((option) => (
              <div
                key={option.id}
                role="button"
                tabIndex={0}
                className={`frontpage-version-item admin-layout-option${option.id === adminLayoutMode ? " active" : ""}`}
                onClick={() => writeAdminLayoutMode(option.id)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    writeAdminLayoutMode(option.id);
                  }
                }}
              >
                <div className="frontpage-version-item-head">
                  ...
                  {option.id === adminLayoutMode && <span className="frontpage-version-badge">Aktiv</span>}
                </div>
