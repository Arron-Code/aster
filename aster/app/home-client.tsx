...
export default function HomeClient({ initialDrinkItems, initialFoodItems, initialIsAdminUser }: HomeClientProps) {
  ...
  return (
    <div className="home-page" style={{ ...themeVars, backgroundColor: settings.bodyBackground, color: settings.bodyTextColor }}>
      <header className="site-header">
        <div className="site-header-bg-slideshow" aria-hidden="true">
          {PUBLIC_HEADER_IMAGES.map((imgUrl, index) => (
            <div
              key={imgUrl}
              className={`header-bg-slide${index === currentBgIndex ? " active" : ""}`}
              style={{
        ...
        <div className="wrap site-header-inner">
          <div className="brand-mark-float" ref={brandMarkRef}>
            <button
              type="button"
        ...
        <div className="wrap header-welcome-row">
          <h2 className="header-welcome-title">{t.welcome}</h2>
          <nav className="menu-tabs" aria-label="Produktkategorien">
            {(Object.keys(menuTabs) as Array<keyof typeof menuTabs>).map((tabKey) => (
              <a
                key={tabKey}
                ...
                onClick={(e) => {
                  e.preventDefault();
                  setActiveTab(tabKey);
                  const target = document.getElementById(`menu-${tabKey}`);
                  if (target) {
                    target.scrollIntoView({ behavior: "smooth" });
                  }
                }}
              >
                <span aria-hidden="true">{tabIcons[tabKey] || "✨"}</span>
                <span>{menuTabs[tabKey].label}</span>
              </a>
            ))}
          </nav>
        </div>
      ...
      <main className="wrap" id="menu">
        <div className="card product-catalog-panel">
          <nav className="menu-tabs-sticky" aria-label="Kategorien Schnellzugriff">
            {(Object.keys(menuTabs) as Array<keyof typeof menuTabs>).map((tabKey) => (
              <a
                key={tabKey}
                className={`menu-tab${activeTab === tabKey ? " active" : ""}`}
