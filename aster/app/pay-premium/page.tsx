  ...
  return (
    <>
      ...
      <main className="wrap payment-page">
        <div className="order-title-row"><div><p className="eyebrow">Tischzahlung</p><h2>Deine Bestellung</h2></div><Link className={`order-status-badge order-status-${(currentOrder?.status || "new").toLowerCase()}`} href={orderTableToken ? `/order-premium?table=${encodeURIComponent(orderTableToken)}` : "/frontpage-premium"}>Weiter bestellen</Link></div>
